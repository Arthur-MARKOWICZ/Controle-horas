...
import { EMPTY_HOLIDAY_CALENDAR } from '../src/shared/holiday-calendar.js'
...
describe('WorkTimeService characterization', () => {
  it('uses the first entry to calculate expected exit', () => {
    ...
    expect(service.expectedExit(logs, '2026-07-14', 530, ['TUESDAY'], false, 0, EMPTY_HOLIDAY_CALENDAR)?.toISOString()).toBe('2026-07-14T20:20:00.000Z')
  })
  it('extends expected exit by completed pauses', () => {
    ...
    expect(service.expectedExit(logs, '2026-07-14', 530, ['TUESDAY'], false, 0, EMPTY_HOLIDAY_CALENDAR)?.toISOString()).toBe('2026-07-14T20:50:00.000Z')
  })
  it('returns null expected exit without entry', () => expect(service.expectedExit([], '2026-07-14', 530, ['TUESDAY'], false, 0, EMPTY_HOLIDAY_CALENDAR)).toBeNull())
  it('uses zero workload on non-work day', () => {
    const logs = [log('2026-07-18T11:30:00Z', null)]
    expect(service.expectedExit(logs, '2026-07-18', 530, ['MONDAY'], false, 0, EMPTY_HOLIDAY_CALENDAR)?.toISOString()).toBe('2026-07-18T11:30:00.000Z')
  })
  ...
  it('includes planned lunch before lunch is registered', () => {
    ...
    expect(service.expectedExit(logs, '2026-07-14', 470, ['TUESDAY'], true, 60, EMPTY_HOLIDAY_CALENDAR)?.toISOString()).toBe('2026-07-14T20:20:00.000Z')
  })
  ...
  it('does not add an open current journey to hour bank', () => {
    expect(service.hourBank([log('2026-07-14T11:00:00Z', null)], 530, ['TUESDAY'], '2026-07-14', '2026-07-14', '2026-07-14', EMPTY_HOLIDAY_CALENDAR)).toBe(0)
  })
  it('includes a past day ending in pause', () => {
    const logs = [log('2026-07-13T11:00:00Z', '2026-07-13T12:00:00Z', 'PAUSE')]
    expect(service.hourBank(logs, 530, ['MONDAY'], '2026-07-13', '2026-07-14', '2026-07-13', EMPTY_HOLIDAY_CALENDAR)).toBe(-470)
  })
  it('accumulates closed balances', () => {
    ...
    expect(service.hourBank(logs, 530, ['MONDAY', 'TUESDAY'], '2026-07-13', '2026-07-14', '2026-07-13', EMPTY_HOLIDAY_CALENDAR)).toBe(20)
  })
  it('debits a past work day without logs', () => expect(service.hourBank([], 530, ['MONDAY'], '2026-07-13', '2026-07-14', '2026-07-13', EMPTY_HOLIDAY_CALENDAR)).toBe(-530))
  it('does not debit days before the configured absence start', () => {
    expect(service.hourBank([], 480, ['MONDAY', 'TUESDAY'], '2026-07-13', '2026-07-15', '2026-07-15', EMPTY_HOLIDAY_CALENDAR)).toBe(0)
  })
  it('counts imported work before the configured absence start against the daily workload', () => {
    ...
    expect(service.hourBank(imported, 480, ['MONDAY'], '2026-07-13', '2026-07-14', '2026-07-14', EMPTY_HOLIDAY_CALENDAR)).toBe(60)
  })
  it('does not debit today without logs', () => expect(service.hourBank([], 530, ['TUESDAY'], '2026-07-14', '2026-07-14', '2026-07-14', EMPTY_HOLIDAY_CALENDAR)).toBe(0))
  it('adds non-work-day effort', () => {
    const logs = [log('2026-07-18T11:00:00Z', '2026-07-18T13:00:00Z')]
    expect(service.hourBank(logs, 530, ['MONDAY'], '2026-07-18', '2026-07-18', '2026-07-18', EMPTY_HOLIDAY_CALENDAR)).toBe(120)
  })
  ...
  it('attributes spillover minutes to the next date', () => {
    ...
    expect(service.hourBank(logs, 0, [], '2026-07-14', '2026-07-15', '2026-07-14', EMPTY_HOLIDAY_CALENDAR)).toBe(60)
  })
  it('respects configured work days', () => {
    expect(service.hourBank([], 60, ['MONDAY'], '2026-07-13', '2026-07-14', '2026-07-13', EMPTY_HOLIDAY_CALENDAR)).toBe(-60)
  })
