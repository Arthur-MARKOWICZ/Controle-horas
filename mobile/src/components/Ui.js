...
export function Field({ label, error, ...props }) { const { theme } = useTheme(); return <View style={styles.field}><Text style={{ color: theme.text, fontWeight: '600' }}>{label}</Text><TextInput placeholderTextColor={theme.muted} style={[styles.input, { color: theme.text, borderColor: error ? theme.danger : theme.border }]} {...props} />{error && <Text style={{ color: theme.danger }}>{error}</Text>}</View> }
